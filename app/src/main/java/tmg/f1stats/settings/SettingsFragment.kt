...
class SettingsFragment : PreferenceFragmentCompat() {
    ...
    private fun year() {
        findPreference<Preference>(keyPreferenceCustomisationYear)?.let { pref ->
            pref.summary = getString(R.string.settings_customisation_year_description, prefs.selectedYear)
            pref.setOnPreferenceClickListener {
                activity?.let { activity ->
                    val years: Array<CharSequence> = supportedYears.map { it.toString() }.toTypedArray()
                    AlertDialog.Builder(activity)
                        ...
                        .setSingleChoiceItems(years, years
                            ...
                            prefs.selectedYear = years[index].toString().toIntOrNull() ?: currentYear
                            pref.summary = getString(R.string.settings_customisation_year_description, prefs.selectedYear)
                            dialog.dismiss()
                        }
                        .create()
