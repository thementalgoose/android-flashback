...
internal class OverviewMapperTest: BaseTest() {
    ...
    private val mockQ1: Map<String, tmg.flashback.formula1.model.RaceQualifyingResult> = mockk(relaxed = true)
    private val mockQ2: Map<String, tmg.flashback.formula1.model.RaceQualifyingResult> = mockk(relaxed = true)
    private val mockQ3: Map<String, tmg.flashback.formula1.model.RaceQualifyingResult> = mockk(relaxed = true)
    private val mockQSprint: Map<String, tmg.flashback.formula1.model.RaceSprintQualifyingResult> = mockk(relaxed = true)
    private val mockQRace: Map<String, tmg.flashback.formula1.model.RaceRaceResult> = mockk(relaxed = true)
    ...
    fun `Season maps fields correctly`() {
        ...
        val expected = tmg.flashback.formula1.model.Season(
            ...
            races = listOf(
                tmg.flashback.formula1.model.Race(
                    season = 2020,
                    round = 1,
    ...
    fun `Season constructor standings is empty list if standings are null`() {
        ...
        val input = FSeason.model(standings = null)
        assertEquals(emptyList<tmg.flashback.formula1.model.SeasonStanding<Constructor>>(), sut.mapSeason(input, 2020).constructorStandings)
    }
    ...
    fun `Season race is empty map if race is null`() {
        ...
        assertEquals(emptyList<tmg.flashback.formula1.model.Race>(), sut.mapSeason(input, 2020).races)
    }
    ...
    fun `Round maps fields correctly`() {
        ...
        val expected = tmg.flashback.formula1.model.Race(
            season = 2020,
            round = 1,
            date = LocalDate.of(2020, 1, 1),
            time = LocalTime.of(12, 0, 0),
