...
fun getWidgetColourData(context: Context, showBackground: Boolean, isDarkMode: Boolean): WidgetConfigurationData {
    return WidgetConfigurationData(
        contentColour = when {
            showBackground && Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && isDarkMode -> Color(
                ContextCompat.getColor(context, android.R.color.system_neutral1_50))
            showBackground && Build.VERSION.SDK_INT >= Build.VERSION_CODES.S -> Color(ContextCompat.getColor(context, android.R.color.system_neutral1_800))
        ...
        backgroundColor = when {
            ...
            showBackground && Build.VERSION.SDK_INT >= Build.VERSION_CODES.S -> Color(ContextCompat.getColor(context, android.R.color.system_accent2_10))
            showBackground -> Color.DarkGray.copy(alpha = 0.5f)
